...
# Para calcular los otros espectros cambiar aca
direccion = glob.glob( camino + '\\ustar.csv')
for j in direccion:
	#Para calcular los otros espectros cambiar aca (el -9 tambien)
	guardado = j[0:-9] + 'ustar\\' 
	...
	for x in long:
		...
		# fi_e ya está elevado a la dos tercios, por eso no lo hago
		espectro['pi_u'] = frec * intensidad / (u_star[i]**2 * fi_e)
		#Ajuste de los datos
		def potencial(a,b,c):
			return b*a**c
		...
		plt.title(nombre + " 1/L = "
			  + str('{:.2e}'.format(obukhov[i]))
			  + " Uo = " + str('{:.2e}'.format(u_star[i]))
			 )
		plt.xlabel("f*z/U")
		# Para calcular los otros espectros cambiar fi-e
		plt.ylabel("f*Su/(Uo^2*fi-e)")				
		plt.legend()
	...
	tabla = {
		...
		'1/L' : obukhov,
		'U' : u_medio,
		# Para calcular los otros espectros tambien cambiar aca
		'Uo' : u_star,								
		'a' : fiteo_a,
		'std_a' : error_a,
		'b' : fiteo_b,
